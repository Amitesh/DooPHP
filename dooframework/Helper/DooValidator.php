...
class DooValidator {
    ...
    public function validate($data=null, $rules=null){
        ...
        foreach($data as $dk=>$dv){
            if($this->requireMode == self::REQ_MODE_NULL_EMPTY && ($dv === null || $dv === '') ||
			   $this->requireMode == self::REQ_MODE_NULL_ONLY  && $dv === null){
                unset($data[$dk]);
			}
        }
        ...
        foreach($data as $k=>$v){
            ...
            foreach($cRule as $v2){
                if(is_array($v2)){
					...
					$vIsEmpty = ($this->requireMode == self::REQ_MODE_NULL_EMPTY && ($v === null || $v === '') ||
								 $this->requireMode == self::REQ_MODE_NULL_ONLY  && $v === null) ? true : false;
                    ...
                    if($err = call_user_func_array(array(&$this, 'test'.$v2[0]), $vv) ){
                        if($this->checkMode==self::CHECK_ALL)
                            $errors[$k][$v2[0]] = $err;
                        else if($this->checkMode==self::CHECK_SKIP && !$vIsEmpty && $v2[0]!='optional'){
                            return $err;
                        }else if($this->checkMode==self::CHECK_ALL_ONE)
                            $errors[$k] = $err;
                    }
     ...
     */
    public function setRequiredFieldDefaults( $displayMethod = self::REQ_MSG_UNDERSCORE_TO_SPACE, $suffix = ' field is required'){
    ...
    public function getRequiredFieldDefaultMsg($fieldname){
        if($this->requiredMsgDefaultMethod==self::REQ_MSG_UNDERSCORE_TO_SPACE)
            return ucfirst(str_replace('_', ' ', $fieldname)) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==self::REQ_MSG_THIS)
            return 'This ' . $this->requiredMsgDefaultSuffix;        
        
        if($this->requiredMsgDefaultMethod==self::REQ_MSG_CAMELCASE_TO_SPACE)
            return ucfirst(strtolower(preg_replace('/([A-Z])/', ' $1', $fieldname))) . $this->requiredMsgDefaultSuffix;
        
        if($this->requiredMsgDefaultMethod==self::REQ_MSG_FIELDNAME)
            return $fieldname . $this->requiredMsgDefaultSuffix;
    }

    public function testOptional($value){}
    public function testRequired($value, $msg){
		if ($this->requireMode == self::REQ_MODE_NULL_EMPTY && ($value === null || $value === '') ||
			$this->requireMode == self::REQ_MODE_NULL_ONLY  && $value === null) {
